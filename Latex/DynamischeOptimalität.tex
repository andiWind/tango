...
\noindent Im eigentlichen Satz wird f $\mathit{OPT}\left(X\right) \geq {_X(T, Y, X)} $ gezeigt werden. Dafür werden aber noch ein Lemma und einige Begriffe benötigt. Der \textbf{linke innere Pfad} $\left(v_0,v_1,..,v_n \right)$ eines Knotens $u$ ist der längst mögliche Pfad für den dem gilt, $v_0$ ist das linke Kind von $u$ und $v_i$ ist rechtes Kind von $v_{i-1}$. Der \textbf{rechte innere Pfad} $\left(v_0,v_1,..,v_n \right)$ eines Knotens $u$ ist der längst mögliche Pfad für den gilt, $v_0$ ist das rechte Kind von $u$ und $v_i$ ist linkes Kind von $v_{i-1}$. $T^r_l$ ist ein von $T$ abgeleiteter BST mit Schlüsselmenge $K^r_l = \{i \in K\vert l \leq i \leq r\}$. Sein nun $v$ ein Knoten in $T$ mit dem linken inneren Pfad $P_l$ und rechtem inneren Pfad $P_r$ und $u$ ein Konten in $T^r_l$ mit dem gleichen Schlüssel wie $v$. Sei $k_l$ der Schlüssel des Knotens $v_l$ mit dem kleinsten Index in $P_l$, so dass gilt $k_l \in K^r_l$, wenn ein solcher Knoten $v_l$ existiert. Existiert $v_l$ nicht, so hat $u$ kein linkes Kind, anderenfalls ist der Knoten mit Schlüssel $v_l$ sein linkes Kind. Das rechte Kind von $u$ wird auf die gleiche Art und Weise aus $P_r$ bestimmt. \\
So erstellt muss $T^r_l$ ein BST sein, so dass für zwei Schlüssel $k_1, k_2 \in K^r_l$ gilt, ist der Knoten mit Schlüssel $k_2$ ein Nachfahre des Knoten mit Schlüssel  $k_1$ in $T^r_l$, dann gilt dies auch für $T$. Direkt ersichtlich ist, dass kein Knoten in  $T^r_l$ mehr als zwei Kinder hat und dass die Links-Rechts-Beziehung der Schlüssel eingehalten wird. Auf drei andere Punkte wird nun noch etwas eingegangen. 
\begin{enumerate}
	\item $T^r_l$ hat eine Wurzel: \\
	 Der Schlüssel $k_r$ des tiefstem gemeinsamen Vorfahren $v_r$ der Knoten mit Schlüsseln aus $K^r_l$ in $T$, muss selbst in $K^r_l$ liegen. Denn hat $v_r$ keine Kinder ist sein Schlüssel der Einzige aus $K^r_l$. Hat $v_r$ ein Kind $v_{rc}$ und $k_r \notin K^r_l$, dann wäre $v_{rc}$ ein tieferer gemeinsamer Vorgänger der entsprechenden Knoten. Hat $v_r$ zwei Kinder mit Schlüsseln aus $K^r_l$ dann muss der von $k_r$ auch in $K^r_l$ enthalten sein. Sei $v_{rc}$ nun das linke bzw. rechte Kind zweier  Kinder von $v_r$, dessen Schlüssel nicht in $K^r_l$ liegt. Gilt zusätzlich $k_r \notin K^r_l$, dann wäre des rechte bzw. linke Kind von $v_r$ ein tieferer gemeinsamer Vorgänger der entsprechenden Knoten. \\
	$v_r$ kann auf keinem inneren Pfad eines anderen Knoten mit Schlüssel aus  $K^r_l$ liegen, damit kann der Knoten mit Schlüssel $k_r$ keinen Vater in $T^r_l$ haben.     
	\item Für die Schlüsselmenge $K$ von  $T^r_l$ gilt  $K \subseteqq  K^r_l$:\\
	 Sei $k \in K^r_l$ und $k < k_r$. Sei $v_k$ der Knoten mit Schlüssel $k$ in $T$. Da $v_r$ ein Vorfahre von $v_k$ ist muss $v_r$ einen Vater $v_p$ mit Schlüssel $k_p$ haben. Gilt $k_p \in K^r_l $ dann ist der Knoten mit Schlüssel $k_p$ Vater des Knotens mit Schlüssel $k$ in $T^r_l$. Ansonsten muss $k > k_p$ gelten und $v_k$ ist ein rechtes Kind. Sei $v_0,v_1,..,v_n$ ein Pfad in $T$ mit $v_0 = v_k$ und $v_i$ ist der Vater von $v_{i-1}$.  
