...
\end{figure}
\paragraph{Grundoperationen Suchen, Einfügen und Löschen} 
Hier geht es nur um die Standardvariante eines BST. Später werden Varianten gezeigt die von diesem Verhalten zum Teil deutlich abweichen. Es sei ein BST $T$ gegeben.\\
 Beim \textbf{Suchen} startet man bei der Wurzel und vergleicht den darin enthaltenen Schlüssel mit dem Gesuchten. Ist der gesuchte Schlüssel kleiner, muss er sich im linken Teilbaum des betrachtetem Knoten befinden und die Suche wird bei dessen Wurzel fortgesetzt. Ist der Schlüssel größer, muss er sich im rechten Teilbaum befinden und die Suche wird bei dessen Wurzel fortgesetzt. Diese Verhalten iteriert man solange bis der gesuchte Schlüssel gefunden ist, oder der Knoten bei dem die Suche fortgesetzt werden müsste, im BST nicht vorhanden ist. Ist das Letztere der Fall, ist der gesuchte Schlüssel im Baum nicht vorhanden. \\
Beim \textbf{Einfügen} eines Schlüssels $k$, führt man zunächst eine Suche nach diesem aus. Findet man den Schlüssel wird das Einfügen abgebrochen. Wurde ein leerer Teilbaum $T_2$ erreicht, wird ein neu erzeugter Knoten mit Schlüssel $k$ an der Position von $T_2$ eingefügt.  \\
\begin{figure}[h]
	\centering
...
\begin{Lemma} Die Höhe eines vollständig balancierten BST $T$ mit $n$ Knoten ist $ \lceil\log_2{(n + 1)} \rceil - 1$. 
\end{Lemma}
\begin{proof}

Bei einem vollständig balancierten BST mit Höhe $h$ sind die oberen $h - 1$ Ebenen vollständig besetzt. Die $h$-te Ebene enthält mindestens einen Knoten, maximal ist auch sie voll besetzt.
Zuerst wird gezeigt, dass  $\lceil\log_2{(n + 1)}\rceil - 2$ Ebenen nicht ausreichen. \\
$\sum \limits_{i=0}^{\lceil\log_2{(n + 1)}\rceil - 2} 2^i = 2^{\lceil\log_2{(n + 1)}\rceil - 1} - 1 \leq 2^{(\log_2{(n + 1)}) - 1} - 1$ \\
Nun wird gezeigt das $ \lceil\log_2{n} \rceil - 1$ Ebenen ausreichend sind.\\
$\sum \limits_{i=0}^{\lceil\log_2{n}\rceil - 1} 2^i =  2^{\lceil\log_2{n}\rceil} - 1 \geq 2^{\log_2{n}} - 1 $
