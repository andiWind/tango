...
Hier geht es nur um die Standardvariante eines BST. Später werden Varianten gezeigt die von diesem Verhalten zum Teil deutlich abweichen. Es sei ein BST $T$ mit Höhe $h$ gegeben. Beim \textbf{Suchen} startet man bei der Wurzel und vergleicht den darin enthaltenen Schlüssel mit dem Gesuchten. Ist der gesuchte Schlüssel kleiner, muss er sich im linken Teilbaum des betrachtetem Knoten befinden und die Suche wird dort fortgesetzt. Ist der Schlüssel größer, muss er sich im rechten Teilbaum befinden und die Suche wird dort fortgesetzt. Diese Verhalten iteriert man solange bis der gesuchte Schlüssel gefunden ist, oder der Knoten bei dem die Suche fortgesetzt werden müsste, im BST nicht vorhanden ist. Ist das Letztere der Fall, ist der gesuchte Knoten im Baum nicht vorhanden. Es werden maximal $h + 1$ Knoten abgearbeitet, die Laufzeit beträgt deshalb $O(h)$. Beim \textbf{Einfügen} eines Schlüssels $k$, führt man zunächst eine Suche nach diesem aus. Findet man den Schlüssel wird das Einfügen abgebrochen. Bricht die Suche ab, da ein leerer Teilbaum $T_l$ erreicht wurde, wird ein neu erzeugter Knoten mit Schlüssel $k$ an der Position von $T_l$ eingefügt.  
