...
Hier werden die verwendeten Zugriffsfolgen zunächst kurz vorgestellt. Auf Details zum Aufbau wird dann im Abschnitt zu dem  jeweiligem Test eingegangen. $n$ entspricht der Anzahl der Knoten und $m$ der Länge der Zugriffsfolge. Als Schlüsselmenge wird $K = \{1,2,..,n\}$ verwendet. Gibt es keine Abhängigkeit von $m$ zu $n$, so haben die Zugriffsfolgen die Länge $40$ Millionen. Die Laufzeittests zu den Punkten $3 - 7$ beziehen sich auf die Eigenschaften eines dynamisch optimalen BST aus Abschnitt \ref{upperBounds}. Hier werden dann Zugriffsfolgen verwendet, bei denen davon ausgegangen werden kann, dass sich die jeweilige Eigenschaft deutlich auf die Laufzeit auswirkt. Folgende Zugriffsfolgen werden verwendet:
 ...
 \begin{enumerate}
    ...
    Hier wird ein Schlüssel $k_F \in K$ gewählt. Die Häufigkeit des Vorkommens eines Schlüssel $k \in K$ in $X$ ist von $\vert k - k_F \vert$ abhängig. Schlüssel zu denen dieser Wert klein ist, sind häufiger enthalten als andere. Es werden solche Zugriffsfolgen für verschiedene $n$ erstellt und die Ergebnisse der beiden Datenstrukturen verglichen. 
    Zusätzlich werden noch Zugriffsfolgen $X_1, X_2,.., X_7$ verwendet, so dass das Static Finger Property mit steigendem Index der Zugriffsfolge weniger relevant wird. Für diese sieben Folgen ist der Wert für $n$ konstant.
 	 \item Zugriffsfolge zum Dynamic Finger Property:
 	Hier werden Zugriffsfolgen verwendet, so dass $\vert x_{i+1} - x_{i} \vert$ für alle $i \in \{1, 2,..,m -1\} $ einen festen parametrierbaren Wert ergibt. Ansonsten wird analog zum Static Finger Property vorgegangen.
 	\item Zugriffsfolge zum Working Set Property:\\
 	In den hier verwendeten Zugriffsfolgen wiederholen sich die Schlüssel mit einem festen parametrierbaren Abstand. Um Einflüsse der anderen aufgeführten Eigenschaften niedrig zu halten, werden die verwendeten Schlüssel betragsmäßig über die gesamte Schlüsselmenge verteilt. Außerdem folgt auf einem Schlüssel der kleiner ist als $n /2$ einer der größer ist und umgekehrt. Ansonsten wird analog zum Static Finger Property vorgegangen.   
  \item Alternierend der kleinste und der größte Schlüssel.
...
\subsubsection{Zufällige Zugriffsfolge}
	Ein Pseudozufallsgenerator erstellt die hier verwendeten Zugriffsfolgen. Für jedes $i \in \{1,2..,m\}$ und $k \in K$ ist die Wahrscheinlichkeit, dass $x_i = k$ gilt, gleich $1 /n$. Es werden jeweils zehn so erzeugte Folgen je verwendeter Schlüsselmenge getestet. Als erstes werden die Zeiten zu den Zugriffsfolgen dargestellt, bei denen der Tango Baum die kürzesten  Laufzeiten erreichte. Dann wird ebenso zum Splay Baum vorgegangen. Außerdem werden noch die Durchschnittswerte aus den jeweils zehn Zugriffsfolgen präsentiert.
\begin{figure}[H]
	\centering
...
\begin{table}[H]
	...
	\end{center}
\end{table}
\subsubsection{Working Set Property}
