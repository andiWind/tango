...
Die Idee hinter dieser Eigenschaft ist, dass es einfacher ist, Zugriffsfolgen schnell auszuführen, wenn ihre Schlüssel betragsmäßig nahe beieinander liegen. Ein BST erfüllt die Static Finger wenn zu jedem Schlüssel $k_f \in K$ für die amortisierte Laufzeit von $X$ 
\begin{align*}
...
\noindent \textbf{Für $T_{i-1}$ gilt Aussage 2:}\\
 $\mathit{bh} \left(T_{i} \right) = \mathit{bh} \left(T_{i-1} \right)$ denn $\mathit{bh} \left(T_{i-1} \right) > \mathit{bh} \left(t_{i} \right)$ und \textit{insertFixup} kann die Schwarz-Höhe eines RBT  ohne rotes Kind der Wurzel nicht erhöhen, vergleiche Abschnitt \ref{if2}. Sollte $v$ durch Schritt $1$ zu einem roten Kind der Wurzel geworden sein, so hat $v$ nach Schritt eins einen schwarzen Elternknoten. Auch in diesem Fall erhöht \textit{insertFixup} die Schwarz-Höhe nicht. Mit  $\mathit{bh} \left(v_{i-1} \right) = \mathit{bh} \left(v_{i} \right)$ muss  $\mathit{bh} \left(v_{i} \right) < \mathit{bh} \left(v_{i+1} \right)$ gelten. Somit ist für $T_i$ die Aussage eins korrekt.\\
...
In diesem Kapitel wird kurz die Implementierung zum Tango Baum beschrieben. Außerdem  werden noch Laufzeittests zwischen dem Tango Baum und dem Splay Baum durchgeführt. 

Implementiert wurde der Tango Baum, mit dem Rot-Schwarz-Baum in der Rolle als Hilfsbaum. Außerdem wurde der Splay Baum implementiert, um Laufzeittests zwischen diesem und dem Tango Baum durchführen zu können. Bedient werden kann das Programm über eine einfach gehaltene graphische Oberfläche. Das Programm wurde mit Java 8 übersetzt und als IDE wurde Apache NetBeans 12.0 verwendet. 
\begin{figure}[H]
	\centering
...
Es werden Laufzeittests zu unterschiedlichen Arten von Zugriffsfolgen durchgeführt. Unter anderem solche, die auf eine der Eigenschaften  Static Finger,  Working Set oder Dynamic Finger aus Abschnitt \ref{upperBounds} zugeschnitten sind. Hierzu wird dann entweder eine Zugriffsfolge verwendet, bei der die zur Eigenschaft gehörende obere Schranke niedrige Werte annimmt oder es wird versucht die Reaktion der Laufzeit auf steigende Werte dieser Schranke zu zeigen. Bei Ersterem werden dann die Laufzeiten in Bezug zur Knotenanzahl dargestellt. Bei Letzterem werden dann mehrere Zugriffsfolgen verwendet, bei denen der Wert der oberen Schranke nach und nach ansteigt. Die Länge der Zugriffsfolgen und die Anzahl der Knoten in den Bäumen ist dabei dann konstant. Außerdem wird mit mehreren Varianten sortierter Zugriffsfolgen getestet. Ein Datenblatt zu dem verwendeten System ist dem Kapitel angefügt. \\

\subsection{Verwendete Zugriffsfolgen zu den Laufzeittests.}
Hier werden die verwendeten Zugriffsfolgen zunächst kurz vorgestellt. Auf Details zum Aufbau wird dann im Abschnitt zu dem  jeweiligem Test eingegangen. $n$ entspricht der Anzahl der Knoten und $m$ der Länge der Zugriffsfolge. Als Schlüsselmenge wird $K = \{1,2,..,n\}$ verwendet. Gibt es keine Abhängigkeit von $m$ zu $n$, so haben die Zugriffsfolgen die Länge $40$ Millionen. Die Laufzeittests zu den Punkten $3 - 7$ beziehen sich auf die Eigenschaften eines dynamisch optimalen BST aus Abschnitt \ref{upperBounds}. Hier werden dann Zugriffsfolgen verwendet, bei denen davon ausgegangen werden kann, dass sich die jeweilige Eigenschaft deutlich auf die Laufzeit auswirkt. Folgende Zugriffsfolgen werden verwendet:

 \begin{enumerate}
 	\item Zufällig erzeugte Zugriffsfolgen.
 	\item Die bit reversal permutation aus Abschnitt \ref{abschnittBitReversal}:\\
 	Es werden mehrere solcher Folgen mit unterschiedlich langen Binärdarstellungen der Schlüssel verwendet.
 	\item Static Finger: \\
    Hier wird ein Schlüssel $k_F \in K$ gewählt. Die Häufigkeit des Vorkommens eines Schlüssel $k \in K$ in $X$ ist von $\vert k - k_F \vert$ abhängig. Schlüssel zu denen dieser Wert klein ist, sind häufiger enthalten als andere.\\
     Zusätzlich werden noch Zugriffsfolgen $X_1, X_2,.., X_7$ verwendet, so dass die Static Finger Eigenschaft mit steigendem Index der Zugriffsfolge weniger relevant wird. Der Splay Baum hat die Static Finger Eigenschaft, deshalb sollten bei ihm die Laufzeiten der Folgen mit höherem Index auch höhere Werte annehmen. 
 	\item Working Set:\\
 	Hier wird eine Zugriffsfolge verwendet in der lediglich Schlüssel aus einer Teilmenge von $K$ enthalten sind. Die Kardinalität dieser Teilmenge wird deutlich kleiner sein als die von $K$. Zu einer solchen Zugriffsfolge sollte die Working Set Eigenschaft dann besonders relevant sein. Auch  werden zusätzliche Zugriffsfolgen $X_1, X_2,.., X_7$ verwendet, für die zur Working Set Eigenschaft analog das gleiche gilt, wie im oberen Punkt zur Static Finger Eigenschaft.\\
 	Die Eigenschaft statisch optimal zu sein wird von der Working Set Eigenschaft impliziert. Diese Eigenschaft wird auch vor allem dann relevant, wenn es kleine Anzahl an Schlüsseln aus $K$ gibt, die bei einem großen Anteil der \textit{access} Operationen als Parameter verwendet werden. Auf zusätzliche Tests zu dieser Eigenschaft wird deshalb verzichtet.     
 \item Dynamic Finger:
  Hier wird eine Zugriffsfolge $X = x_1, x_2, ..., X_m$ verwendet werden, so dass $\vert x_{i+1} - x_{i} \vert$ für alle $i \in \{1, 2,..,m -1\} $ einen kleinen Wert ergibt. Außerdem werden wieder Zugriffsfolgen $X_1, X_2,.., X_7$ verwendet, mit denen analog zu denen aus Static Finger vorgegangen wird.
 \end{enumerate}



\subsubsection{Durchführung der Laufzeittests.} 

\subsubsection{Zufällige Zugriffsfolge}
	Ein Pseudozufallsgenerator erstellt eine Zugriffsfolge $X = x_1, x_2,.., x_m$ für diesen Test. Für jedes $i \in \{1, 2,..,m\}$ und jedes $k \in K$ ist die  Wahrscheinlichkeit, dass $x_i = k$ gilt, gleich $1/n$.
\begin{figure}[H]
	\centering
...
\begin{figure}[H]
	...
	\caption{Diagramm zum Laufzeittest zu Static Finger.}
\end{figure}
\begin{table}[H]
	\begin{center}
		\begin{tabular}[c]{|c|c|c|}
			...
			\hline
		\end{tabular}
		\caption{Die Laufzeiten zum  Test zu Static Finger.} 
	\end{center}
\end{table}
...
Beim ersten Test wird Zugriffsfolge $1, 3, 5,..,n-1, 1, 3, 5, .., n-1,...$ verwendet. Der Abstand $a$ zwischen zwei aufeinanderfolgenden Schlüsseln ist \mbox{also $2$}.
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{"Medien/laufzeittest/diagramm/dynamicfinger"}
	\caption{Diagramm zum Laufzeittest zu Dynamic Finger mit konstantem $a$.}
\end{figure}
\begin{table}[H]
	\begin{center}
		\begin{tabular}[c]{|c|c|c|}
...
Es ist zu erkennen, dass die Zeit des Splay Baums ansteigt, obwohl $n$ und $m$ konstant sind.
\begin{figure}[H]
	\centering
...
\begin{table}[H]
	...
	\end{center}
\end{table}
\noindent Hier ist $n = 1.000.000$ fest und der Prozentwert $p$ für die Anzahl der Schlüssel im working set ist veränderlich. Wieder steigen die Zeiten des Splay Baumes leicht an.
\begin{figure}[H]
	\centering
