...
\noindent Es gilt $\mathit{bh} \left(t_i \right) \leq \mathit{bh} \left(v_{i} \right)$, $\mathit{bh} \left(t_i \right) < \mathit{bh} \left(v_{i+1} \right)$ und $\mathit{bh} \left(v_{l} \right) - \mathit{bh} \left(v_{0} \right) = O\left(log\left(n\right)\right) $.\\
Für $T_i =$ \textit{concatenate}$\left(T_{i-1}, \textit{key}(v_i), t_i\right)$, gilt entweder $\mathit{bh} \left(T_{i} \right) = \max\{\mathit{bh} \left(T_{i-1}\right), \mathit{bh} \left(t_{i} \right) \} $ oder  $\mathit{bh} \left(T_{i} \right) = \max\{\mathit{bh} \left(T_{i-1}\right), \mathit{bh} \left(t_{i} \right) \} + 1$.\\
 Ist das letztere der Fall, hat die Wurzel von $T_i$ kein rotes Kind, denn entweder ist $\mathit{bh} \left(T_{i-1}\right) = \mathit{bh} \left(t_{i} \right)$, oder \textit{insertFixup} färbte, mit Fall $2$,  beide Kinder schwarz. In dieser Konstellation könnte $\mathit{bh} \left(T_i \right) \geq \mathit{bh} \left(t_{i+1} \right)$ gelten. Für $T_{i+1} =$ \textit{concatenate}$\left(T_{i}, \textit{key}(v_{i+1}), t_{i+1}\right)$ muss dann jedoch $\mathit{bh} \left(T_{i+1} \right) = \max\{\mathit{bh} \left(T_{i}\right), \mathit{bh} \left(t_{i+1} \right) \} $  gelten. Denn \textit{insertFixup} erhöht nur in Fall $2$ die Schwarz-Höhe eines Knoten, dessen Kinder müssten jedoch rot sein, vergleiche Abschnitt \ref{if2}.\\
 
 
 
 
 Es folgt $\mathit{bh} \left(T_{i} \right) \leq \mathit{bh} \left(v_{i+1} \right) + 1$.  Daraus folgt $\mathit{bh} \left(T_{i+1} \right) \leq \mathit{bh} \left(v_{i+2} \right) $.
