...
Ein neuer Schlüssel wird zunächst wie in Ref: (normales Einfügen) eingefügt. Zusätzlich werden dann noch die Zeiger auf die Kinder auf den schwarzen Sonderzeiger gesetzt. Der neue Knoten wird rot gefärbt. Durch den neu eingefügten Knoten können Korrekturen notwendig werden, um die Rot-Schwarz-Baum Eigenschaften zu erhalten. Zunächst betrachten wir die fünf Eigenschaften um zu erkennen welche betroffen sein können. Es ist immer noch jeder Knoten entweder rot oder schwarz. Aufgrund des Sonderknotens sind die Blätter immer noch schwarz. Da der neue Knoten rot ist bleiben auch die Schwarz-Höhen der Knoten unverändert, so dass Eigenschaft fünf erhalten bleibt. Wurde in den leeren Baum eingefügt, so wurde jedoch eine rote Wurzel eingefügt. Ist der Vater des neuen Knotens rot, so wurde Eigenschaft vier verletzt. Zur Korrektur wird eine zusätzliche Routine verwendet. Diese arbeitet sich von der Einfügestelle solange nach oben durch, bis alle Eigenschaften wieder eingehalten werden. Pro Iteration werden acht Fälle unterschieden, von denen jeweils zwei symmetrisch sind.
