...
Hier geht es nur um die Standardvariante eines BST. Später werden Varianten gezeigt die von diesem Verhalten zum Teil deutlich abweichen. Es sei ein BST $T$ gegeben.\\
  ...
  Beim \textbf{Einfügen(Schlüssel $k$)} wird zunächst eine Suche nach $k$ ausgeführt. Findet man den Schlüssel wird das Einfügen abgebrochen und der BST bleibt unverändert. Wurde ein leerer Teilbaum $T_2$ erreicht, wird ein neu erzeugter Knoten mit Schlüssel $k$ an der Position von $T_2$ eingefügt.  \\
\begin{figure}[h]
	\centering
...
Auch beim \textbf{Löschen(Schlüssel $k$)} wird zunächst eine Suche nach diesem ausgeführt. Ist $k$ im BST nicht vorhanden wird abgebrochen und der BST bleibt unverändert. Ansonsten werden drei Fälle unterschieden.
\begin{enumerate}
