...
Außerdem kann die angegebene obere Schranke mit Hilfe der Interleave Bound nicht weiter verbessert werden. Denn wird \textit{access} auf einen Schlüssel $k$ ausgeführt der im preferred path mit der Wurzel enthalten ist, so entstehen im Tango-Baum Kosten $O\left(\log\left(\log\left(n\right)\right)\right)$, es kann aber maximal zu einem Interleave kommen, nämlich durch den Knoten mit Schlüssel $k$. 
