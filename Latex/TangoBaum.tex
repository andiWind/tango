...
\end{enumerate}
Zu klären ist noch warum im sechsten Punkt der richtige Hilfsbaum gefunden werden muss. Sei $v_p$ der Knoten in $P$ mit Schlüssel $k$. Vor dem durchführen von Punkt sechs ist man immer in der Situation, dass \textit{maxDepth} der Wurzel von $H_3$ der Tiefe von $v_p$ entspricht. Man möchte nun $H_4$ so hinzufügen, wie es einem linken preferred child von $v_p$ entspricht. Sei $v_l$ das linke Kind von $v_p$. Es muss einen Zeiger aus $H_3$ auf den Hilfsbaum geben in dem der Schlüssel \textit{key}$\left(v_l\right)$ enthalten ist geben.
