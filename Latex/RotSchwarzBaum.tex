...
Zu Beginn des Kapitels wurde erwähnt, dass für die Höhe $h$ eines RBT mit $n$ Knoten $\mathit{O(n)}$ gilt. Das wird nun gezeigt. 
\newtheorem{Lemma}{Lemma}[section]
\begin{Lemma} Für die Höhe $h$ eines RBT $T$ mit $n$ Knoten gilt $h \leq  2 lg(n + 1)$. 
\end{Lemma}
\begin{proof}
	Sei $w$ die Wurzel von $T$
    ...
    \begin{align*}
    &2^\mathit{bh(w)- 1} - 1  + 1  + 2^\mathit{bh(w)- 1} - 1\\
   =&2^\mathit{bh(w)} - 1
    \end{align*}
    
    
    Für die Höhe $x_h$ eines Knoten $x$ gilt folgender Zusammenhang:\\
    \begin{align*}
    x_h \leq 2 * \mathit{bh(x) } + 1
    \end{align*}
    Denn höchstens jeder zweite Knoten in einem Pfad kann rot sein.\\
    Es gilt also:\\
    $n \geq 2^\mathit{bh(w)} - 1$
