...
public class StaticFingerTest extends RuntimeTest {
  ...
  private double percent;
     ...
     */
    public StaticFingerTest(int n, double p, int r, RuntimeFrame rf, boolean d){
   ...
   * Auf den mittleren Schlüssel der Schlüsselmenge entfallen "percent"% der Zugriffe. Auf den nächst kleineren und größeren Schlüssel entfallen insgesamt 2% der restlichen
   * Zugriffe usw.
   ...
   */          
     long[] startTest()  throws BuildAuxTreeFaildException{
         //Es wird eine Zugriffsfolge erzeugt bei der p% der Zugriffe auf den mittleren Schlüssel entfallen. Dann p% der restlichen Zugriffe auf die beiden Schlüssel mit Abstand 1 
        ...
        nodeArray[midKey][0] = (int)(percent * lOs / 100);
        ...
        for (int i = 1; i < midKey; i++){
            if(exit)
                return null;
             int numOfAccess = (int)(percent * lOs / 100);
